...
require("../../../config.php");
	...
	function listIdeas(){
		$notice = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("SELECT id, idea, ideacolor FROM vpuserideas WHERE userid = ? AND deleted IS NULL ORDER BY id DESC");		echo $mysqli->error;
...
//<p style="background-color: #ff2f2f">Kõik peavad rõõmust osa saama! | <a href="edituserideas.php?id=17">Toimeta</a></p>
		}
	...
	function latestIdea(){
		//$ideaHTML = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("SELECT idea FROM vp3userideas WHERE id = (SELECT MAX(id) FROM vpuserideas WHERE deleted IS NULL)");
		echo $mysqli->error;
